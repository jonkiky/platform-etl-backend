...
<yandex>
    <profiles>
        <default>
		    ...
		    <max_bytes_before_external_sort>28000000000</max_bytes_before_external_sort>
		    <max_bytes_before_external_group_by>28000000000</max_bytes_before_external_group_by>
...
echo "Cloning repository to startup scripts"
git clone https://github.com/opentargets/platform-etl-backend.git
SCRIPTS=/platform-etl-backend/clickhouse

echo "Creating table and loading data"
clickhouse-client --multiline --multiquery < ${SCRIPTS}/aotf_log.sql

# Load data
gsutil -m cp -r gs://ot-snapshots/etl/outputs/${RELEASE}/aotf/clickhouse .
cd clickhouse || exit
FOLDERS=$(ls -1 | grep json)
for folder in $FOLDERS; do
cat ${folder} | clickhouse-client --query="insert into ot.associations_otf_log format JSONEachRow"
done

clickhouse-client --multiline --multiquery < ${SCRIPTS}/aotf.sql

echo touching $OT_RCFILE
echo "data_version=${RELEASE}" > $OT_RCFILE
date >> $OT_RCFILE